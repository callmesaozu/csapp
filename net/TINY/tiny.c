...
void doit(int fd) 
{
    ...
    if (strcasecmp(method, "GET"))  //目前只能处理GET命令
    ...
    if (stat(filename, &sbuf) < 0)  //读取filename状态到sbuf， 如果成功则返回0，出错则返回-1， 并设置errno
...
int parse_uri(char *uri, char *filename, char *cgiargs) 
{
    ...
    if (!strstr(uri, "cgi-bin"))    //如果"cgi-bin"在uri中出现过
